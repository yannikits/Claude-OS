...
- ~~**catalog.json / catalog.lock.json Schema + Validator deferred** zur Phase-6-Sidecar-Integration. `catalog list/uninstall/enable/disable/update/lock/sync` zeigen Phase-6-Pointer.~~ → **erledigt 2026-05-17**: **Phase 5i** (Commit `cff079c`) schema + store + `list`; **Phase 5l** (Commit `4fa4f7d`) `enable/disable/uninstall` real via `setCatalogEntryEnabled` + `removeCatalogEntry`; **Phase 5m** (Commit folgt) `lock` real via neuem `lockCatalog`-Builder (github-Sources fetch+sha256+cache, marketplace/local skip mit warning, bindings=[] bis Phase 5n). Restlich Phase-6-Hint: `update`, `sync` (komponieren auf `lock`).
