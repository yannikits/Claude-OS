/**
 * Map NinjaOne HTTP/network failures to `BridgeResult.kind`.
 *
 *   401 / 403  → auth-failed
 *   429        → rate-limited
 *   404        → misconfigured (organizationId likely wrong)
 *   5xx        → unreachable
 *   abort/conn → unreachable
 *   else       → error
 *
 * @module @domains/msp-bridges/ninja/classify-error
 */
...
export function classifyHttpStatus(
  ...
  if (status === 404) {
    return { kind: 'misconfigured', message: 'HTTP 404 — organizationId unknown to NinjaOne' };
  }
